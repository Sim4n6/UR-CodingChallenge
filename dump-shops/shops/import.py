...
for i, entity in enumerate(bson.decode_file_iter(f)):
    ...
    result = conn.execute(
        text(
            """SELECT id FROM shops WHERE name = :name AND email = :email AND coordinate_lat = :coordinate_lat AND coordinate_long = :coordinate_long; """
        ),
        {
            "name": name,
            "email": email,
    ...
    if result == None:
        ...
        conn.execute(
            text(
                """ INSERT INTO shops (picture, name, email, city, coord_type, coordinate_lat, coordinate_long) VALUES (:picture, :name, :email, :city, :coord_type, :coordinate_lat, :coordinate_long)"""
            ),
            {
                "picture": picture,
                "name": name,
